...
// The swapchain is float16 by the params above, so this answers scRGB; the
// other cases are kept for a host that asks for something else one day.
...
static bool sp_init(struct ra_ctx *ctx)
{
    ...
    struct d3d11_swapchain_opts scopts = {
        ...
        // ask, and the host reads these textures as float16 scRGB in every
        // case. See ra_d3d11_sockpuppet_set_swapchain_params().
...
{
    // Always 16-bit float, which libplacebo pairs with scRGB (linear light,
    // BT.709, 1.0 = SDR white). Left to its own choice libplacebo picks a
    // 10-bit swapchain for SDR as well as for HDR, and the host that reads
    // these textures (Chromium) opens a shared handle only as 8-bit RGBA or
    // BGRA, or as float16. Float16 is the one of those that loses nothing:
    // an SDR picture rendered into it carries more precision than the
    // 10-bit one the windowed context presents, and an HDR picture is the
    // same scRGB mpv presents with --d3d11-output-format=rgba16f.
