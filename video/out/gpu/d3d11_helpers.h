...
struct d3d11_swapchain_opts {
    ...
    // Don't ask the swapchain's output what format and colour space it would
    // like. A swapchain created for composition is attached to no output, so
    // the query only fails and logs. Callers with a window leave this unset.
    bool skip_output_probe;
