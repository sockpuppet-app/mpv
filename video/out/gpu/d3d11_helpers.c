                                        ...
                                        struct pl_color_space *configured_csp)
{
    ...
    // A swapchain created for composition is attached to no output, so
    // there is nothing there to query and the probe only logs a failure.
    if (!skip_output_probe) {
